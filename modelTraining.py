...
TEST_DIR = '/Users/jiananliu/ml/kaggle/Kaggle-Rental-Listing-Inquireies'
TRAIN_FILE_NAME = os.path.join(TEST_DIR, 'input/train.json')
TEST_FILE_NAME = os.path.join(TEST_DIR, 'input/test.json')
target_num_map = {'high': 0, 'medium': 1, 'low': 2}
train_data = pd.read_json(TRAIN_FILE_NAME).reset_index()
test_data = pd.read_json(TEST_FILE_NAME).reset_index()
list_img_time = pd.read_csv(os.path.join(TEST_DIR, "input/listing_image_time.csv"))
